...
class WOO_Slg_Public {
    ...
    public function woo_slg_login_otp_email() {
        ...
        if( isset($_POST['otp']) && !empty($_POST['otp']) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST[ 'woo_slg_otp_nonce' ] ) ), 'woo_slg_otp_verify' ) ) {
            ...
            if( $user_id ) {
                ...
                if( trim($actual_otp) == trim($otp) ) {
                    delete_user_meta($user_id, '_woo_slg_activation_code');
            ...
            $response['error'] = 1;
            $response['message'] = esc_html__( 'Unauthorized access.', 'wooslg' );
            echo json_encode( $response );
            //do exit to get proper response
            exit;
        }
    ...
    public function woo_slg_confirm_email_user() {
        ...
        if( !empty($_GET["woo_slg_verify"]) ) {
            ...
            if( !empty($woo_slg_verify_array['id']) && !empty($woo_slg_verify_array["code"]) ) {
                ...
                $check_status = get_user_meta($woo_slg_verify_array['id'], '_woo_slg_user_activate', true);

                if( ( !empty($stored_code) && $stored_code == $woo_slg_verify_array["code"] ) && ( $check_status != 'activated' ) ) {
